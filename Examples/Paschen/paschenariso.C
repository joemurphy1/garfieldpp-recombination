...
int main(int argc, char* argv[]) {
  ...
  std::vector<double> pdFe = {
      0.71755, 0.9843,  1.42324, 1.95233, 2.93121,
      4.88989, 9.77222, 19.5293, 29.3212, 48.9141,
      97.7525, 195.354, 237.575};
  std::vector<double> vbFe = {
      277.775, 261.486, 259.518, 267.479, 286.297,
      330.484, 427.263, 600.244, 747.254, 1010.87,
      1602.59, 2618.59, 3000};
  std::vector<double> pdNi = {
      0.97692, 1.42324, 1.96708, 2.93121, 4.92683, 9.77222,
      19.5293, 29.3212, 48.9141, 97.7525, 195.354, 237.575};
  std::vector<double> vbNi = {
      194.76,  202.257, 216.486, 244.299, 299.572, 414.545,
      595.726, 747.254, 1010.87, 1602.59, 2618.59, 3000};
  std::vector<double> pdPt = {
      0.77948, 0.9843,  1.40198, 1.95233, 2.93121,
      4.88989, 9.77222, 19.3829, 22.873};
  std::vector<double> vbPt = {
      196.237, 191.84,  194.76,  206.893, 229.972,
      279.882, 384.384, 552.382, 614};
  std::vector<double> pdBa = {
      0.97692, 1.36041, 1.95233, 2.93121, 4.92683, 9.77222,
      19.5293, 29.3212, 48.9141, 98.4909, 195.354, 210.623};
  std::vector<double> vbBa = {
      115.646, 127.579, 146.161, 173.896, 226.524, 330.484,
      493.206, 642.472, 909.426, 1485.98, 2465.03, 2579.33};
  std::vector<double> pdNa = {
      0.50378, 0.70683, 0.99173, 1.42324, 1.98194,
      2.93121, 4.92683, 9.77222, 19.5293, 29.3212,
      48.9141, 98.4909, 195.354, 210.623};
  std::vector<double> vbNa = {
      90.1294, 91.5014, 98.6813, 113.912, 133.495,
  ...
  for (int i = 0; i < fIso.size(); ++i) {
    ...
    std::string gasfile = "ar-" + std::to_string(fAr) + 
                          "-ic4h10-" + std::to_string(fIso[i]) + "-p1.gas";
    ...
    g->Draw("lsame");
    char buf[100];
    snprintf(buf, 100, "Magboltz, Ar-iC_{4}H_{10} %d-%d, #gamma = 10^{%g}",
             fAr, fIso[i], log10(gamma));
