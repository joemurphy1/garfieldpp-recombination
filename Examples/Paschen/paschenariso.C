...
int main(int argc, char* argv[]) {
  ...
  TCanvas* c1 =
      new TCanvas("c1", "Paschen curves for Ar/iC_{4}H_{10}", 21, 28, 500, 527);
  c1->SetLogx();
  c1->SetLogy();
  ...
  double pdFe[] = {0.71755, 0.9843,  1.42324, 1.95233, 2.93121,
                   4.88989, 9.77222, 19.5293, 29.3212, 48.9141,
                   97.7525, 195.354, 237.575};
  double vbFe[] = {277.775, 261.486, 259.518, 267.479, 286.297,
                   330.484, 427.263, 600.244, 747.254, 1010.87,
                   1602.59, 2618.59, 3000};
  double pdNi[] = {0.97692, 1.42324, 1.96708, 2.93121, 4.92683, 9.77222,
                   19.5293, 29.3212, 48.9141, 97.7525, 195.354, 237.575};
  double vbNi[] = {194.76,  202.257, 216.486, 244.299, 299.572, 414.545,
                   595.726, 747.254, 1010.87, 1602.59, 2618.59, 3000};
  double pdPt[] = {0.77948, 0.9843,  1.40198, 1.95233, 2.93121,
                   4.88989, 9.77222, 19.3829, 22.873};
  double vbPt[] = {196.237, 191.84,  194.76,  206.893, 229.972,
                   279.882, 384.384, 552.382, 614};
  double pdBa[] = {0.97692, 1.36041, 1.95233, 2.93121, 4.92683, 9.77222,
                   19.5293, 29.3212, 48.9141, 98.4909, 195.354, 210.623};
  double vbBa[] = {115.646, 127.579, 146.161, 173.896, 226.524, 330.484,
                   493.206, 642.472, 909.426, 1485.98, 2465.03, 2579.33};
  double pdNa[] = {0.50378, 0.70683, 0.99173, 1.42324, 1.98194,
                   2.93121, 4.92683, 9.77222, 19.5293, 29.3212,
                   48.9141, 98.4909, 195.354, 210.623};
  double vbNa[] = {90.1294, 91.5014, 98.6813, 113.912, 133.495,
                   162.466, 216.486, 325.528, 493.206, 642.472,
                   909.426, 1485.98, 2465.03, 2579.33};

  for (int i = 0; i < sizeof(pdFe) / 8; i++) {
    pdFe[i] /= 760.0;
  }
  for (int i = 0; i < sizeof(pdNi) / 8; i++) {
    pdNi[i] /= 760.0;
  }
  for (int i = 0; i < sizeof(pdPt) / 8; i++) {
    pdPt[i] /= 760.0;
  }
  for (int i = 0; i < sizeof(pdBa) / 8; i++) {
    pdBa[i] /= 760.0;
  }
  for (int i = 0; i < sizeof(pdNa) / 8; i++) {
    pdNa[i] /= 760.0;
  }
  ...
  for (int igas = 0; igas < 3; igas++) {
    ...
    if (igas == 0) {
      char buf[100];
      snprintf(buf, 100, "Magboltz, Ar-iC_{4}H_{10} 99-1, #gamma = 10^{%g}",
               log(gamma) / log(10.0));
      leg->AddEntry(g, buf, "l");
    } else if (igas == 1) {
      char buf[100];
      snprintf(buf, 100, "Magboltz, Ar-iC_{4}H_{10} 95-5, #gamma = 10^{%g}",
               log(gamma) / log(10.0));
      leg->AddEntry(g, buf, "l");
    } else if (igas == 2) {
      char buf[100];
      snprintf(buf, 100, "Magboltz, Ar-iC_{4}H_{10} 90-10, #gamma = 10^{%g}",
               log(gamma) / log(10.0));
      leg->AddEntry(g, buf, "l");
    }
  }
