...
polyline_pl::polyline_pl(polyline& pl) {
  if (pl.Gqsl() < 2) {
    std::cerr << "error in polyline_pl(polyline& pl): qsl=" << Gqsl();
    spexit(std::cerr);
  }
...
polygon::polygon(const straight* fsl, int fqsl, double prec)
  ...
  for (n = 1; n < fqsl; n++) {
    ...
    if (vecerror != 0) {
      std::cerr << "error in polygon::polygon(straight* fsl, int fqsl):\n"
...
rectangle::rectangle(const point& fpiv, vec fdir[2], double fdim[2],
  ...
  if (check_perp(fdir[0], fdir[1], prec) != 1) {
    ...
    // perpendicular sides.
    // std::cerr << "fdir[2](directions of sides):\n" << fdir[0] << fdir[1];
    spexit(std::cerr);
  }
  if (fdim[0] <= 0 || fdim[1] <= 0) {
    ...
    // std::cerr << "fdim (dimensions):" << fdim[0] << ' ' << fdim[1] << '\n';
    // std::cerr << "fdir[2](directions of sides):\n" << fdir[0] << fdir[1];
    spexit(std::cerr);
  }
  piv = fpiv;
