...
namespace Garfield {
  ...
  void AvalancheGridSpaceCharge::Set2dGrid(const double zmin, const double zmax, const int zsteps,
                                           const double rmax, const int rsteps) {
  ...
  void AvalancheGridSpaceCharge::AvalancheElectron(const double x, const double y, const double z,
  ...
  void AvalancheGridSpaceCharge::StartGridAvalanche(double dtime) {
    // avalanche the electrons until a certain delta-time OR there are no electrons left in the gap
    ...
    if (m_bImportAvalanche && !m_bPreparedImportAvalanche) {
      ...
      if (m_bDebug)
        std::cerr << m_className
                  << "::StartGridAvalanche Microscopic electrons successfully prepared.\n";
    ...
    if (m_vNElectronEvolution.size() > 0) {
      // determine maximal size of electron maxSize at time maxTime.
      auto maxSize = std::max_element(m_vNElectronEvolution.begin(), m_vNElectronEvolution.end(),
                                      [](const std::pair<double, long> &p1, const std::pair<double, long> &p2) {
                                        return p1.second < p2.second;
                                      });
      double maxTime = m_AvGrid.time0 + m_AvGrid.dt *
                                        ((double) std::distance(m_vNElectronEvolution.begin(), maxSize) + 1);

      std::cout << m_className
                << "::StartGridAvalanche::Avalanche maximum size of " << maxSize->second
                << " electrons reached at " << maxTime << " ns.\n";

      std::cout << m_className
                << "::StartGridAvalanche::Final avalanche size (produced positive charge) = "
                << m_AvGrid.nTotPosIons
                << " ended at t = " << m_AvGrid.time << " ns.\n";
    }
  }
  ...
  void AvalancheGridSpaceCharge::ExportGrid(const std::string &filename) {
    ...
    for (int iz = 0; iz <= m_AvGrid.zSteps; iz++) {
      for (int ir = 0; ir <= m_AvGrid.rSteps; ir++) {
        nd = &m_GridMesh[iz][ir];
        gasGap = nd->gasGapIndex;
        EField = nd->eFieldZ + m_vEFieldZBackgroundGasLayer[gasGap];
    ...
    for (int iz = 0; iz <= m_AvGrid.zSteps; iz++) {
      for (int ir = 0; ir <= m_AvGrid.rSteps; ir++) {
        nd = &m_GridMesh[iz][ir];
        gasGap = nd->gasGapIndex;
        EField = std::sqrt(std::pow(nd->eFieldZ + m_vEFieldZBackgroundGasLayer[gasGap], 2)
    ...
    if (m_bDebug) {
      std::cerr << m_className << "::ExportGrid Grids exported.\n";
    }
  }

  void AvalancheGridSpaceCharge::ImportEllipticIntegralValues(const std::string &filename) {
    // reads values of the elliptic functions

    /// so far it has a very special form to find the values rapidly (not given for a completely non uniform grid)
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////
  /// Private Section:
  bool AvalancheGridSpaceCharge::SnapTo2dGrid(const double x, const double y, const double z,
                                              const long n, const int gasLayer) {
    ...
    if (indexZ < 0 || indexZ > m_AvGrid.zSteps || indexR < 0 || indexR > m_AvGrid.rSteps) {
      if (m_bDebug) {
        std::cerr << m_className << "::SnapTo2dGrid: Point is outside the grid.\n";
      }
      return false;
    }
    ...
    // additional electrons or get attached. (depends on if against E field or along ...)
    ...
    if (m_GridMesh[indexZ][indexR].gasGapIndex != gasLayer) {
      std::cerr << m_className << "::SnapTo2dGrid: Gas layer index does not match.\n";
      return false;
    }
    ...
    if (!against) {
      ...
      if (m_bDebug)
        std::cerr << m_className << "::SnapTo2dGrid: snap along e-field, continue.\n";
      return true;
    }
    ...
    GetAvalancheSizeFromStep(std::abs(step), n, m_GridMesh[indexZ][indexR].townsend,
                             m_GridMesh[indexZ][indexR].attachment, nEOut, nPosOut, nNegOut);
    if (nEOut == 0) {
      if (m_bDebug)
        std::cerr << m_className << "::SnapTo2dGrid: e- from " << n << " to 0 -> cancel.\n";
      return false;
    }
    ...
    if (m_bDebug) {
      std::cerr << m_className << "::SnapTo2dGrid: e- from " << n << " to " << nEOut
                << " p+: " << nPosOut << " n-: " << nNegOut << ".\n"
                << "    Snapped to (z, r) = (" << y << " -> " << m_AvGrid.zGrid[indexZ]
                << ", " << r << " -> " << m_AvGrid.rGrid[indexR] << ").\n";
    }
    return true;
  }
  ...
  void AvalancheGridSpaceCharge::Prepare2dMesh() {
    ...
    if (!m_sensor) {
      std::cerr << m_className << "::Prepare2dMesh Sensor is not defined. Abort.\n";
    }
    ...
    std::vector<double> alpha(n), eta(n), drift(n), dSigmaL(n), dSigmaT(n), wv(n), wr(n), alphaPT(n), etaPT(n);
    double e[3], v;
    int status;
    Medium *m = nullptr;
    // iterate through the gas gaps
    for (k = 0; k < n; k++) {
      m_sensor->ElectricField(0, m_vYPointInGasGap[k], 0, e[0], e[1], e[2], v, m, status);
    ...
    for (int iz = 0; iz <= m_AvGrid.zSteps; iz++) {
      ...
      for (int ir = 0; ir <= m_AvGrid.rSteps; ir++) {
        ...
        if (k == -1) {
          m_GridMesh[iz][ir].isGasGap = false;
          continue;
        }
        // set swarm parameters & time
        m_GridMesh[iz][ir].townsend = alpha[k];
        m_GridMesh[iz][ir].attachment = eta[k];
        m_GridMesh[iz][ir].velocity = drift[k]; //< magnitude! direction against E_field. E/|E|
        m_GridMesh[iz][ir].dSigmaL = dSigmaL[k];
        m_GridMesh[iz][ir].dSigmaT = dSigmaT[k];
        m_GridMesh[iz][ir].Wv = wv[k];
    ...
    int izMin, izMax, izAnode;
    for (k = 0; k < n; k++) {
      izMin = m_AvGrid.zGasGapBoundaries[k].front();
      izMax = m_AvGrid.zGasGapBoundaries[k].back();
      izAnode = (m_vEFieldZBackgroundGasLayer[k] > 0) ? izMin : izMax;
      for (int ir = 0; ir <= m_AvGrid.rSteps; ir++) {
  ...
  void AvalancheGridSpaceCharge::PrepareElectronsFromMicroscopicAvalanche() {
    ...
    for (int k = 0; k < (int) m_vIndexGasGaps.size(); k++) {
      // calculate middle coord of electron cloud and add all electrons to the grid/mesh per gas gap.
      ...
      for (auto &electron: m_vElectrons[k]) {
        xMicro += electron.x;
        yMicro += electron.y;
        zMicro += electron.z;
        tMicro += electron.t;
      ...
      if (m_bDebug) {
        std::cerr << m_className << "::ImportElectronsFromMicroscopicAvalanche mean center gas gap "
                  << k + 1 << " X = ("
                  << m_vCoNGasLayer[k][0] << ", " << m_vCoNGasLayer[k][1] << ", " << m_vCoNGasLayer[k][2]
                  << ").\n";
      }
    }
    ...
    for (int k = 0; k < (int) m_vIndexGasGaps.size(); k++)
      for (auto &electron: m_vElectrons[k]) { //< does not proceed if no electrons in respective gap
        if (SnapTo2dGrid(electron.x, electron.y, electron.z, 1, k) && m_bDebug) {
          std::cerr << m_className
                    << "::GetElectronsFromAvalancheMicroscopic::Electron added in gas gap " << k + 1 << "\n"
                    << "      at (x,y,z) =  (" << electron.x << "," << electron.y << "," << electron.z
                    << ").\n";
        }
      }
  }

  void AvalancheGridSpaceCharge::GetSwarmParameters(const double MagEField, double &alpha, double &eta, double &drift,
    ...
    drift = std::sqrt(vx * vx + vy * vy + vz * vz);  //< is Wv in Magboltz interpretation
    // wv, wr <- take only Wr and 'drift' for Wv
    wr = 0.;
    if (!m_bUseTOF || !m->ElectronVelocityFluxBulk(0., MagEField, 0., 0., 0., 0., wv, wr) || wr < Small) {
      m_bWrAvailable = false;
      wr = drift;
    }
    // rates, if not available we take (alpha-eta)SST and ratio of alpha/eta = Rion/Ratt Rion-Ratt = Reff (tagashira eq.)
  ...
  bool AvalancheGridSpaceCharge::TransportTimeStep() {
    // Transport GridMesh one time-step with updated E-fields (Lippmann et al. approach)
    if (!m_AvGrid.run) return false;

    // local helper variables
    int gasGap;
    long nElectronOut;
    double nPosIonOut, nNegIonOut, MagEField, step, stepZ, stepR;
    GridNode *nd;
    ...
    std::function<void(double, const long, const double, const double, long &, double &,
                       double &)> AvalancheGain = GetAvalancheSizeFromStep;
    ...
    if (m_bSpaceCharge && m_AvGrid.nTotElectron > 1e5) {
      for (int iz = 0; iz <= m_AvGrid.zSteps; iz++) {
        ...
        gasGap = m_GridMesh[iz][0].gasGapIndex;
        if (gasGap == -1)
          continue;
        for (int ir = 0; ir <= m_AvGrid.rSteps; ir++) {
          nd = &m_GridMesh[iz][ir];
          ...
          // calculate field at the current bin from all other bins containing charge
          GetLocalField(iz, ir, nd->eFieldZ, nd->eFieldR, m_sFieldOption, gasGap);

          // check if local field reaches background field values.
          MagEField = std::sqrt(std::pow(nd->eFieldZ + m_vEFieldZBackgroundGasLayer[gasGap], 2)
                                + std::pow(nd->eFieldR, 2));
          if (MagEField - std::abs(m_vEFieldZBackgroundGasLayer[gasGap]) >=
            ...
            std::cout << m_className << "::Space-charge field reached "
                      ...
                      << "% of background field in gas gap "
                      << gasGap + 1
                      << "\n";
          ...
          step = std::abs(nd->Wr * m_AvGrid.dt);

          // adaptive time stepping (this routine takes the smallest dt needed for the current sc-field)
          if (m_bAdaptiveTime && step >= 2. * m_AvGrid.zStepSize) {
            ...
            if (m_bDebug) {
              std::cerr << m_className << "::TransportTimeStep Changed dt from "
                        ...
                        << "      due to step size: " << step << " bulk velocity: " << nd->Wr << "\n"
                        << "      electric field: " << MagEField << " alpha: " << nd->townsendPT
                        << " eta: " << nd->attachmentPT << "\n"
      ...
      for (int ir = 0; ir <= m_AvGrid.rSteps; ir++) {
        ...
        if (!m_bSpaceCharge &&
            (std::find(m_vSaturatedGaps.begin(), m_vSaturatedGaps.end(), gasGap) != m_vSaturatedGaps.end()
             ? true : false)) {
          nElectronOut = nd->nElectron; //< saturated case, don't evolve electrons in size
          nPosIonOut = 0, nNegIonOut = 0; //< strictly this is completely wrong because SC-bremsung creates huge amounts of ions
        } else {
          AvalancheGain(step, nd->nElectron, nd->townsendPT, nd->attachmentPT, nElectronOut, nPosIonOut,
                        nNegIonOut);
        }
        ...
        MagEField = std::sqrt(
                std::pow(nd->eFieldZ + m_vEFieldZBackgroundGasLayer[gasGap], 2) + std::pow(nd->eFieldR, 2));
        stepZ = step * (-(nd->eFieldZ + m_vEFieldZBackgroundGasLayer[gasGap]) / MagEField);
        stepR = step * (-(nd->eFieldR) / MagEField);

        if (m_bDiffusion) {
          // correct the stepping from diffusion + charge distribution
          DiffuseTimeStep(step, nElectronOut, std::round(nPosIonOut), std::round(nNegIonOut),
                          iz, ir, gasGap);
        } else {
          // calculate steps and distribute charges (no diffusion)
          DistributeCharges(nElectronOut, std::round(nPosIonOut), std::round(nNegIonOut),
                            iz, ir, stepZ, stepR, gasGap);
        }

        if (m_sensor->GetNumberOfElectrodes() > 0) {
          // adding the movement to signal: (go to global cartesian coordinates using phi = 0)
    ...
    // propagate Grid time (after above for loops due to the adaptive time stepping)
    ...
    m_vNElectronEvolution.push_back(std::make_pair(m_AvGrid.time, m_AvGrid.nTotElectron));
    // continue run if electrons left on grid
    m_AvGrid.run = m_AvGrid.nTotElectron > 0;

    // determine saturated gaps at each time step
    m_vSaturatedGaps.resize(0); //< clear: anode-absorption activates avalanche to grow again
  ...
  void AvalancheGridSpaceCharge::DiffuseTimeStep(double dx, long nElectron, double nPosIon, double nNegIon,
    ...
    double MagEField, sinTheta, cosTheta;
    ...
    // if electron are less than m_dMinGroups * GroupSize(-1) then we diffuse each electron by itself:
    ...
    for (int group = 0; group < groups; group++) {
      ...
      DistributeCharges(groupSize,
                        nPosIon * (double) groupSize / (double) nElectron,
                        nNegIon * (double) groupSize / (double) nElectron,
                        iz, ir, stepZ, stepR,
                        gasGap);  // < fractional ion number is allowed otherwise loss of ions
    }
  }

  void AvalancheGridSpaceCharge::DistributeCharges(long nElectron, double nPosIon, double nNegIon,
                                                   int iz, int ir, double stepZ, double stepR, int gasGap) {
    // distributes the charges from a movement in Z and R direction

    stepR = std::abs(m_AvGrid.rGrid[ir] + stepR) - m_AvGrid.rGrid[ir]; // effective step size in r-direction
    double zRatio = (m_AvGrid.zGrid[iz] + stepZ - m_AvGrid.zGrid[0]) / m_AvGrid.zStepSize;
    double rRatio = std::abs(m_AvGrid.rGrid[ir] + stepR) / m_AvGrid.rStepSize; // can travel through r=0
    ...
    double az, bz, ar, br;
    int izPost2, irPost2;
    izPost2 = izPost + signZstep;
    // always: irPost2 >= 0
    irPost2 = irPost + signRstep;

    bz = std::abs(zRatio - (double) izPost);
    az = 1 - bz;
    br = std::abs(rRatio - (double) irPost);
    ar = 1 - br;
    ...
    // check if still in grid else place at boundaries (will be absorbed in next step)?
    int izMin = m_AvGrid.zGasGapBoundaries[gasGap].front();
    int izMax = m_AvGrid.zGasGapBoundaries[gasGap].back();
    ...
    // add to the nodes the electrons travelled to (into nElectronHolder as they will mix)
    if (nElectron > 200) {
      m_GridMesh[izPost][irPost].nElectronHolder += (long) std::round((double) nElectron * az * ar);
      m_GridMesh[izPost][irPost2].nElectronHolder += (long) std::round((double) nElectron * az * br);
      m_GridMesh[izPost2][irPost].nElectronHolder += (long) std::round((double) nElectron * bz * ar);
      m_GridMesh[izPost2][irPost2].nElectronHolder += (long) std::round((double) nElectron * bz * br);

      // add positive ions to the nodes (smeared values allowed)
      m_GridMesh[izPost][irPost].nPosIonHolder += nPosIon * az * ar;
      m_GridMesh[izPost][irPost2].nPosIonHolder += nPosIon * az * br;
      m_GridMesh[izPost2][irPost].nPosIonHolder += nPosIon * bz * ar;
      m_GridMesh[izPost2][irPost2].nPosIonHolder += nPosIon * bz * br;

      // add negative ions to the nodes (smeared values allowed)
      m_GridMesh[izPost][irPost].nNegIonHolder += nNegIon * az * ar;
      m_GridMesh[izPost][irPost2].nNegIonHolder += nNegIon * az * br;
      m_GridMesh[izPost2][irPost].nNegIonHolder += nNegIon * bz * ar;
      m_GridMesh[izPost2][irPost2].nNegIonHolder += nNegIon * bz * br;

    } else {
      // too large movement of few electrons -> only move it to 1 node (instead of 4)
      izPost = (az >= bz) ? izPost : izPost2;
      irPost = (ar >= br) ? irPost : irPost2;

      m_GridMesh[izPost][irPost].nElectronHolder += nElectron;
      m_GridMesh[izPost][irPost].nPosIonHolder += nPosIon;
      m_GridMesh[izPost][irPost].nNegIonHolder += nNegIon;
    }
  }

  void AvalancheGridSpaceCharge::GetLocalField(const int iz, const int ir,
                                               double &eFieldZ, double &eFieldR, const std::string &fieldOption,
                                               int gasGap) {
    ...
    if (fieldOption == "coulomb") {
      ...
      // assume symmetric single layer rpc with equal permittivity resistive layers.
      if (!m_bImportElliptic) {
        throw std::runtime_error("::GetLocalField Elliptic values not imported.");
      }

      if (m_vIndexGasGaps.size() > 1) {
        throw std::runtime_error("::GetLocalField Mirror charge option implemented but not tested for MRPC.");
      }

      // get the rpc (ComponentParallelPlate)
      auto *rpc = m_ParallelPlate;
      double N, eps = 1., alpha12, beta12, resThickness, zf0; //< neighbored resistive layer thickness from where?
        ...
        for (int fr = 0; fr <= m_AvGrid.rSteps; fr++) {
          ...
          for (int i = 0; i < m_iFieldApprox; i++) {
            if (i == 0) {
              // 2a, alpha12 = delta_Q
              zf0 = zf + 2. * (zTop - zf);
              AddFieldFromChargeAt(iz, ir, zf0, rf, N * alpha12, eFieldZ, eFieldR);
  ...
  bool AvalancheGridSpaceCharge::AddFieldFromChargeAt(int iz, int ir, int fz, int fr, double N, double &eFieldZ,
                                                      double &eFieldR) {
    // charge of interest at f, point of interest at i
    ...
    double intermediateEz = 0., intermediateEr = 0., dist;
  ...
  void AvalancheGridSpaceCharge::GetFreeChargedRing(int iz, int ir, int fz, int fr,
                                                    double &eFieldZ, double &eFieldR) {
    // calculate the electric field at point (zi, ri) form charged ring at (zf, rf)
  ...
  void AvalancheGridSpaceCharge::GetFreeChargedRing(double zi, double ri, double zf, double rf,
                                                    double &eFieldZ, double &eFieldR) {
    // calculate the electric field at point (zi, ri) form charged ring at (zf, rf)
    ...
    a2 = (ri + rf) * (ri + rf) + dz * dz;
    b2 = (ri - rf) * (ri - rf) + dz * dz;
    b = std::sqrt(b2);
    c2 = ri * ri - rf * rf - dz * dz;
    // parameter for elliptic integrals
    x = -4 * ri * rf / b2; //< x < 0, i.e. never near x = 1 (singularity)
  ...
  void AvalancheGridSpaceCharge::GetGlobalCoordinates(double r, double z, double phi,
  ...
  void AvalancheGridSpaceCharge::GetEllipticIntegrals(double x, double &K, double &E) {
    // from x = 0 to 10 it is in steps of 1e-3. From 10 to 1e4 in steps of 1. Then in steps of 1000 until 1e7.
    int arg;
    double stepSize;
    ...
    if (-x < 1.e1) {
      ...
      if (m_bDebug) std::cerr << m_className << "::GetEllipticIntegrals value not included in list.\n";
      K = m_vKElliptic.back();
      E = m_vEElliptic.back();
      return;
    }
