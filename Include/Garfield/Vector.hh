...
namespace Garfield
{
  template<typename type,std::size_t Dimension> class Vector: public GarfieldArray<type,Dimension>
  {
    public:
    template <typename... Args> __host__ __device__ Vector(Args &&... args) : GarfieldArray<type,Dimension>({std::forward<Args>(args)...}) {}
  ...
  template<typename type> class Vec1Impl : public Vector<type,1>
  {
    public:
    Vec1Impl() = default;
    template<typename... xs> __host__ __device__ Vec1Impl(xs... values) : Vector<type,3>({values...}){}
  ...
  template<typename type> class Vec2Impl : public Vector<type,2>
  {
    public:
    Vec2Impl() = default;
    template<typename... xs> __host__ __device__ Vec2Impl(xs... values) : Vector<type,3>({values...}){}
    __host__ __device__ type& x() noexcept { return this->operator[] (0);}
    __host__ __device__ type  x() const noexcept { return this->operator[](0);}
  ...
  template<typename type> class Vec3Impl : public Vector<type,3>
  {
    public:
    Vec3Impl() = default;
    template<typename... xs> __host__ __device__ Vec3Impl(xs... values) : Vector<type,3>({values...}){}
    __host__ __device__ type& x() noexcept { return this->operator[] (0);}
    __host__ __device__ type  x() const noexcept { return this->operator[](0);}
    __host__ __device__ type& y() noexcept { return this->operator[] (1);}
    __host__ __device__ type  y() const noexcept { return this->operator[](1);}
