...
class ComponentNeBem2d : public Component {
 ...
 private:
  ...
  double BoxPotential(const double a, const double b, const double x,
                      const double y, const double v0) const;
  /// Field of a uniformly charged rectangle.
  void BoxField(const double a, const double b, const double x, const double y,
                double& ex, double& ey) const;

  void Reset() override;
  void UpdatePeriodicity() override;
  /// Rotation from global to local coordinates.
  void ToLocal(const double xIn, const double yIn, const double cphi,
               const double sphi, double& xOut, double& yOut) const;
  /// Rotation from local to global coordinates.
  void ToGlobal(const double xIn, const double yIn, const double cphi,
                const double sphi, double& xOut, double& yOut) const;
