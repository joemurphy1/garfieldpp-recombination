...
Alternatively, they can be set explicitly using 
\texttt{TrackSrim::SetWorkFunction} and \texttt{TrackSrim::SetFanoFactor}.

The \texttt{Cluster} objects returned by \texttt{TrackSrim::GetClusters} 
contain the location and time of the cluster (variables \texttt{x, y, z, t}), the energy spent to make the cluster (\texttt{energy}), the ion energy when the cluster was created (\texttt{kinetic}), and the number of electrons in the cluster (\texttt{n}). 
...
The variables contained in the \texttt{Cluster} objects returned by \texttt{TrackTrim::GetClusters} are the same as for \texttt{TrackSrim}.
