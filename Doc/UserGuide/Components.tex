...
The key functions, used internally in the classes for simulating charge transport, are 
\begin{lstlisting}
...
The class \texttt{ComponentComsol} can be used for 
...
\begin{lstlisting}
void SetMesh(const std::size_t nx, const std::size_t ny,
             const std::size_t nz, const double xmin, const double xmax,
...
\begin{lstlisting}
void SetMedium(const std::size_t i, Medium* m);
\end{lstlisting}
\begin{description}
...
For visualizing the mesh imported from a FEM field map (or a TCAD field map), the class 
...
\end{lstlisting}
then draws the drift lines stored in the \texttt{ViewDrift} class together with the mesh.
If the flag \texttt{twod} is set to \texttt{true}, 
a two-dimensional projection of the drift lines and a cut view of the mesh at the currently set viewing plane are drawn. 
If \texttt{twod} is \texttt{false}, the drift lines and mesh elements are drawn in 3D. If \texttt{outline} is set to \texttt{true}, only the 
facets separating two regions are drawn. 
...
(suppose that \texttt{driftView} is a pointer to a \texttt{ViewDrift} object) 
\begin{lstlisting}
  ComponentAnsys123 fm;
  // ...
  TCanvas* c1 = new TCanvas();
...
A wire must not be positioned at the origin if polar coordinates are being used.
...
bool MultipoleMoments(const std::size_t iw, const std::size_t order = 4,
                      ...
                      const double eps = 1.e-4, const std::size_t nMaxIter = 20);
\end{lstlisting} 
\begin{description}
...
\begin{lstlisting}
bool ForcesOnWire(const std::size_t iw, 
...
\begin{lstlisting}
void SetNumberOfShots(const std::size_t n);
...
bool OptimiseOnTrack(const std::vector<std::string>& groups,
    ...
    const std::size_t nP = 20, const bool print = true);
\end{lstlisting}
...
bool OptimiseOnGrid(const std::vector<std::string>& groups,
    const std::string& field_function, const double target,
    const double x0, const double y0, const double x1, const double y1,
    const std::size_t nX = 10, const std::size_t nY = 10,
    const bool print = true);
\end{lstlisting}
...
bool OptimiseOnWires(const std::vector<std::string>& groups,
    const std::string& field_function, const double target,
    const std::vector<std::size_t>& wires, const bool print = true);
\end{lstlisting}
...
void SetMinMaxNumberOfElements(const std::size_t nmin, const std::size_t nmax);
\end{lstlisting}
\begin{description}
...
\end{description}
As is the case with \texttt{ComponentUser}, the active volume can be set using \texttt{SetArea} and \texttt{SetMedium} (if it is a box), or using \texttt{SetGeometry} in case of more complex geometries.
...
field/potential to be plotted is set through the constructor of \texttt{ViewField} or by means of
\begin{lstlisting}
void SetComponent(Component* c);
void SetSensor(Sensor* s);
\end{lstlisting}

By default, the voltage range is retrieved from the minimum and maximum 
values of the potential in the component/sensor, and the range of the 
electric and weighting fields is ``guessed'' by taking random samples.
...
double IntegrateFluxCircle(const double xc, const double yc, 
                           const double r, const std::size_t nI = 50);
\end{lstlisting}
