...
void GetComponent(const std::size_t i, std::string& label, double& f);
\end{lstlisting} 
...
in cm\(^{2}\)\,V\(^{-1}\)\,s\(^{-1}\). 
Mobility files for positive ions can be imported using
\begin{lstlisting}
...
bool GetLevel(const std::size_t i, int& ngas, int& type, std::string& descr, double& e);
\end{lstlisting}
\begin{description}
...
\begin{lstlisting}
std::size_t GetNumberOfElectronCollisions();
std::size_t GetNumberOfElectronCollisions(std::size_t& nElastic, std::size_t& nIonising, 
                                          std::size_t& nAttachment, std::size_t& nInelastic, 
                                          std::size_t& nExcitation, std::size_t& nSuperelastic);
std::size_t GetNumberOfElectronCollisions(const std::size_t level);
\end{lstlisting}
