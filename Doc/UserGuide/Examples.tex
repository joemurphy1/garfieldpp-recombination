...
\end{lstlisting}
By default, \texttt{DriftLineRKF} will then include the ion tail in the simulation. 
%\begin{lstlisting}
%drift.EnableIonTail();
%\end{lstlisting}
...
\end{lstlisting}
Next we create a \texttt{Sensor} based on the field map component.
\begin{lstlisting}
Sensor sensor(&fm);
\end{lstlisting}
...
in the \texttt{Data} directory of the project source tree and are copied to the \texttt{install} directory during installation. 
If you don't provide a full path or if the mobility file is not in your current working directory, \texttt{MediumMagboltz} will look for it in the subdirectory \texttt{share/Garfield/Data} of the \texttt{install} folder.
\begin{lstlisting}
gas.LoadIonMobility("IonMobility_Ar+_Ar.txt");
\end{lstlisting}
...
%To describe the shape of our detector, we create a \texttt{SolidBox} object.
%\begin{lstlisting}
%// Thickness of the silicon [cm]
%constexpr double d = 100.e-4;
%SolidBox box(0, 0.5 * d, 0, 2 * d, 0.5 * d, 2 * d);
%\end{lstlisting}
%We then create a \texttt{GeometrySimple} object, and attach the box to it (\ie we pass it a pointer to the \texttt{SolidBox} object), together with the medium inside (\ie a pointer to the \texttt{MediumSilicon} object).
%\begin{lstlisting}
%// Set up the geometry.
%GeometrySimple geo;
%geo.AddSolid(&box, &si);
...
\subsubsection{Electric field}
The active volume in our example is a box with a length of $d = 100$\,\textmu{m} along $y$, centred at $y = 50$\,\textmu{m}, and made of silicon. 
