...
the integral of the current is equal to the collected charge.
...
\begin{lstlisting}
void DriftLineRKF::SetSignalAveragingOrder(const std::size_t navg);
void AvalancheMC::SetSignalAveragingOrder(const std::size_t navg);
\end{lstlisting} 
...
\begin{lstlisting}
Shaper(const std::size_t n, const double tau, const double g, std::string shaperType);
\end{lstlisting}
...
\section{Delayed signals}
For detectors with resistive elements, the induced signal as function of time is not only given by the movement of the charges in the drift medium but also by the time dependent reaction of the resistive materials. 
Computing the induced current in such a configuration requires an extended version of the Ramo-Shockley theorem, 
\begin{equation}\label{Eqn:ExtendedRamoShockleyWeightingField}
i\left(t\right) = -q\int\limits_{0}^{t}\text{d}t'\mathbf{H}\left[\mathbf{x}\left(t'\right), t - t'\right]\cdot \dot{\mathbf{x}}\left(t'\right), 
\end{equation}
where 
\begin{equation*}
\mathbf{H}_{w}\left(\mathbf{x}, t\right) = -\nabla \frac{\partial\phi_{w}\left(\mathbf{x}, t\right)}{\partial t}
\end{equation*}
is the time-dependent weighting vector.

The time-dependent weighting potential $\phi_{w}\left(\mathbf{x}, t\right)$
can be written as the sum of a static prompt component and a dynamic delayed component
\begin{equation}
\phi_{w}\left(\mathbf{x}, t\right) = \phi_{w}^{p}\left(\mathbf{x}\right) + \phi_{w}^{d}\left(\mathbf{x}, t\right), \qquad \text{where}~\phi_{w}^{d}\left(\mathbf{x}, 0\right) = 0.
\end{equation} 
We can then write Eq.~\eqref{Eqn:ExtendedRamoShockleyWeightingField} as 
\begin{equation}\label{Eqn:PromptDelayedCurrent}
i\left(t\right) = \underbrace{-q\mathbf{E}_{w}\left(\mathbf{x}\left(t\right)\right)\cdot \dot{\mathbf{x}}\left(t\right)}_{\text{direct induction}} \underbrace{-q\int\limits_{0}^{t}\text{d}t'\mathbf{H}_{w}^{d}\left[\mathbf{x}\left(t'\right), t - t'\right]\cdot \dot{\mathbf{x}}\left(t'\right)}_{\text{reaction from resistive material}}.
\end{equation}

The computation of the delayed component of the induced current, \textit{i. e.} the second term in Eq.~\ref{Eqn:PromptDelayedCurrent}, can be activated or deactivated using the function
\begin{lstlisting}
void Sensor::EnableDelayedSignal(const bool on);
\end{lstlisting}
To be able to compute the delayed component, the dynamic weighting potential or weighting field of the electrode must be defined. The method for doing so depends on the type of \texttt{Component}. At present, \texttt{ComponentComsol}, \texttt{ComponentGrid}, \texttt{ComponentTcad2d}, \texttt{ComponentTcad3d} and \texttt{ComponentUser} support time-dependent weighting fields/potentials.
